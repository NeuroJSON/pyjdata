function (doc) {
  ...
  if (doc['.datainfo']) {
      datainfo['.datainfo'] = doc['.datainfo'];
  }
  ...
  emit(doc._id, {name: datainfo.Name, length: rawtext.length, readme: txt.substr(0, 256), info: datainfo, subj: subjlist, modality: modalitylist, aisummary: (doc['.datainfo'] && doc['.datainfo'].AISummary) ? doc['.datainfo'].AISummary : ''});
}