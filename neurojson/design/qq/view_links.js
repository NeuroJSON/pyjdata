function (doc) {
  ...
  const jsonpath = /:(\$[^&]+)/;
  const hashpat = /hash=([a-z0-9]+):([0-9a-f]+)/;
  const dotpat = /\./g;
          ...
          if(subkey == '_DataLink_' && typeof obj[subkey] === 'string' && obj[subkey].indexOf('http')!== -1) {
              ...
              if(!urlhash.hasOwnProperty(uniqurl)) {
                ...
                if(jpath !== null && jpath.length)
                    jpath = jpath[1];
                urlhash[uniqurl] = 1;
                var hm = uniqurl.match(hashpat);
                emit([doc._id, fname[3], size], {path: rootpath, url: uniqurl, file: fname[2] + fname[3], suffix: fname[3], ref: jpath, algo: hm ? hm[1] : '', hash: hm ? hm[2] : ''});
              }
          }
